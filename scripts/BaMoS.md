...
BaMoS stands for Bayesian Model Selection. Here are the articles describing the method:

C. H. Sudre, M. J. Cardoso, W. H. Bouvy, G. J. Biessels, J. Barnes and S. Ourselin. **Bayesian Model Selection for 
Pathological Neuroimaging Data Applied to White Matter Lesion Segmentation.** *IEEE Transactions on Medical Imaging*
(2015); 34(10):2079-2102. doi: [10.1109/TMI.2015.2419072](https://doi.org/10.1109/TMI.2015.2419072).

Sudre CH, Gomez Anson B, Davagnanam I, Schmitt A, Mendelson AF, Prados F, Smith L, Atkinson D, Hughes AD, Chaturvedi N, 
Cardoso MJ, Barkhof F, Jaeger HR, Ourselin S. **Bullseye's representation of cerebral white matter hyperintensities.**
*J Neuroradiol.* 2018;45(2):114-122. doi: [10.1016/j.neurad.2017.10.001](https://doi.org/10.1016/j.neurad.2017.10.001).

## BaMoS outputs

The outputs from BaMoS are as follows:

| File pattern                   | Description                                      |
|--------------------------------|--------------------------------------------------|
| CorrectLesion_<subject>.nii.gz | WMH segmentation with improbable lesions removed |
| Layers_<subject>.nii.gz        | White matter segmented into layers               |
| Lobes_<subject>.nii.gz         | White matter segmented into lobes                |

The lobe labels are as follows:
...
The layers are numbered 1-5 from the ventricles outwards. Layers 4 & 5 are the same layer and can be merged.

![image](assets/BaMoS_example.jpg)

> Fig. 1 Representation of the building blocks of the local WMH lesion loads. The first column reflects the lesion 
> segmentation. The second column refers to the separation according to the lobar regions and the last column to the 
> distance based layer separation from the ventricular surface towards the cortical sheet. The lesion frequency per 
> defined local region is then summarized in the bullseye plot. Most central parts correspond to the most 
> periventricular regions. The lobar regions are represented according to the angular position and referred to by their 
> first letters. The subject is male, 75 years old.

*From Sudrè et al. (2017)*

## Installing BaMoS

You can find BaMoS source code at [https://github.com/csudre/BaMoS](https://github.com/csudre/BaMoS)

You should be able to compile BaMoS with the following commands:

```bash
mkdir /path/to/BaMoS/build
cd /path/to/BaMoS/build
cmake /path/to/BaMoS/source/code
make
make install
```

You will also need to install `bc`, the linux basic calculator.
...
various registration steps as well as the main BaMoS binaries `Seg_BiASM` and `Seg_Analysis`. You will need to customise 
these for your particular project.

After the main script there are two postprocessing scripts to run:

### Correction script

Removes improbable lesion voxels.
